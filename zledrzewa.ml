...
let make l ((x, y) as k) r =
    Node (l, k, r, (max (height l) (height r) + 1,
        safe_add ((number_of_elements l) (number_of_elements r) (y - x + 1)))
...
let rec join_separate l v r =
  match (l, r) with
    (Empty, _) -> add_separate cmp v r
  | (_, Empty) -> add_separate cmp v l
...
(* bez zmian *)
(* t1 musi miec wszystkie elementy mniejsze od najmniejszego elementu t2 *)
let merge t1 t2 =
  match t1, t2 with
  | Empty, _ -> t2
  | _, Empty -> t1
...
let remove (x, y) set =
    let (t1, _, _) = split x set and
    let (_, _, t2) = split y set in
    merge t1 t2

(* analogicznie do remove,
split x set to przedzialy mniejsze od x (polaczone w drzewo t),
split y set to przedzialy wieksze od y,
wyniki mozna polaczyc funkcja join_separate *)
